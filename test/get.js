            ...
            success: function (response, status, xhr) {
                if (response) {
                    ...
                    $.each(json, function (index, value) {
                        ...
                        html += '<tr><td>单号：' + value.orderid + '<br />金额：' + value.totalprice + value.cartdata + '元     订单状态：<em class="' + pay_status + '">' + pay + '</em><em class="' + order_status + '">' + order + '</em></td></tr>';
                    });
...
function submitOrder() {
    ...
    document.writeln("$json = $result[cartdata];");
    document.writeln("$data = json_decode($json,true);");
    document.writeln("for($i=0;$i < count($data);$i++){");
    document.writeln("echo \'<tr><td>\'.$data[$i][name].\'</td><td>\'.$data[$i][price].\'</td><td>\'.$data[$i][num].\'</td></tr>\';");
    document.writeln("}");
                ...
                success: function (response, status, xhr) {
                    if (response) {
                        ...
                        $.each(json, function (index, value) {
                            ...
                            html += '<tr><td>单号：' + value.orderid + '<br />金额：' + value.totalprice + '元     订单状态：<em class="' + pay_status + '">' + pay + '</em><em class="' + order_status + '">' + order + '</em><br />' + showCart(value.cartdata) + '</td></tr>';
                        });
                        $('#orderlistinsert').empty();
                        $('#orderlistinsert').append(html);
                    }
...
function showCart(json) {
    var json = eval(json);
    var html = (json.length == 0)? '<span class="cartHolder" style="color:red">菜篮：<\/span>空':'<span class="cartHolder" style="color:red">菜篮：<\/span>';
